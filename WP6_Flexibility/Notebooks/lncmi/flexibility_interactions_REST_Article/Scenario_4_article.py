...
def scenario(df = None, weight = 1, lp:bool = True , cop = 3, copper_coeeff = 0.045, elec_therm_ratio = 0.85, pv_co2 = 36, hp_cf = 0.2):
    ...
    # exp = f" + lpSum(heat_pump_elec_cons_u[t] for t in time.I) <= {limit}"                                            
    ...
    elec_node.connect_units(grid_imp, grid_exp,  surplus_consumption.elec_consumption_unit, heat_pump.elec_consumption_unit,pv)
    ...
    gwp_storage = gwp_storage = ( gwp /lca_years) *  (len(df.resample("1D").mean())/365)  #gco2/kwh
    ...
    lca_years = 25
    gwp_PV =( 1e6 /lca_years) *  (len(df.resample("1D").mean())/365)  #gco2/kwh

    
    
    pv_gwp_obj = Objective(
        name='pv_embodied_co2',
        exp=f'{pv.name}_nb_unit * {gwp_PV}',
        weight=1,
        parent=pv
    )

    setattr(pv, 'pv_gwp_obj', pv_gwp_obj)
    ...
    df["grid_import_S4_kWh"] = grid_imp.p.get_value()
    df["grid_export_S4_kWh"] = grid_exp.p.get_value()
    df["OPEX_S4_€"] = df[["grid_import_S4_kWh", "electricity_price[€/kWh]"]].product(axis = 1) - df[["grid_export_S4_kWh", "elec_export_price[€/kWh]"]].product(axis = 1)

    df["PV_Cap_S4_KWp"] =pv.nb_unit.get_value()

    df["Buffer_Cap_S4_KWp"] =buffer.capacity.get_value()
    df["Buffer_Power_S4_KW"] = buffer.p.get_value()
    df["Buffer_Energy_S4_KW"] = buffer.e.get_value()
    
    df["heatpump_elec_Power_S4_KW"] = heat_pump.elec_consumption_unit.p.get_value()
    df["heatpump_thermal_Power_S4_KW"] = heat_pump.thermal_production_unit.p.get_value()
    df["heatpump_elec_binary_S4"] = heat_pump.elec_consumption_unit.u.get_value()
    
    df["surplus_elec_cons_Power_S4_kW"] = surplus_consumption.elec_consumption_unit.p.get_value()
    df["surplus_therm_prod_Power_S4_kW"] = surplus_consumption.thermal_production_unit.p.get_value()

    df["estimated_CO2_S4_elec"] = grid_imp.co2_emissions.get_value()
    df["estimated_CO2_S4_district_heat"] = district_heat.co2_emissions.get_value()
    df["estimated_CO2_S4_buffer"] = buffer.discharge.co2_emissions.get_value()
    df["estimated_CO2_S4_pv"] = pv.co2_emissions.get_value()
    # df["estimated_CO2_S4_heatpump"] = heat_pump.thermal_production_unit.co2_emissions.get_value()

    df["estimated_CO2_S4_heatpump"] = df["heatpump_elec_Power_S4_KW"] * df["Taux de Co2"] * hp_cf

    df["estimated_CO2_S4_total"] = df[["estimated_CO2_S4_elec","estimated_CO2_S4_district_heat",
                                       "estimated_CO2_S4_buffer" , "estimated_CO2_S4_pv"]].sum(axis =1).tolist()

    df["Consumption_S4"] = temp.sum(axis =1).tolist()
    df["Consumption_S4_compare"] = total_consumption.get_value()
    df["PV_production_S4"] = df.pv.copy() * round(pv.nb_unit.get_value(),2)
    df["heat_demand_district_heat_S4_kW"] = district_heat.p.get_value()
    df["heat_demand_district_heat_S4_MW"] = df["heat_demand_district_heat_S4_kW"] /1000
...
if __name__ == "__main__":
    ...
    for d_set in d_sets: 
        ...
        df, stats = scenario(df, weight = 1,pv_co2 = d_set["PV_CO2"], hp_cf = d_set["hp_cf"])

        with open(f'data/stats/Scenario_4_{d_set["country"]}.txt', 'w') as f:
            f.write(str(stats))
        
        df.to_csv(file, sep=",", decimal=".", index=True, index_label="datetime")

        print (f"Optimization Scenario 4 completed, results saved to {file}")