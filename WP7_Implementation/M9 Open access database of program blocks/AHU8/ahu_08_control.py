...
"""
================================================================================
 AHU_08 - Air Handling Unit Control (Python soft-PLC)   group "RoofTop_Heating"
================================================================================
 Functional alternative to the licensed CoDeSys 3.5 ST program (AHU_08.st),
 mirroring the same logic on a 200 ms cycle.

 Source : 10ahus_selection_AHU8.json  (project "10AHUs")

 Unit type: roof-top HEATING-only supply unit.
   ...
   heating coil (3-way) with frost protection + pump. No cooling, no recovery.

 Fresh-air damper interlock: the fan only starts once the damper proves OPEN;
 failure to prove open within a timeout latches a damper fault.
================================================================================
"""
...
class IOImage:
    ...
    DI_FreshDamper_Open: bool = False      # 2-point fresh-air damper OPEN feedback
    DI_Reset: bool = False
    DI_RemoteEnable: bool = True

    # ---- Analog inputs ----
    AI_T_Supply_C: float = 14.0
    AI_RH_Supply_pct: float = 40.0
    AI_T_Exhaust_C: float = 21.0
    AI_T_Room_C: float = 18.0
...
class AHUController:
    ...
    def scan(self, io: IOImage, dt_s: float = P.DT_S) -> None:
        ...
        if s in (State.OFF, State.SHUTDOWN):
            ...
            io.AO_HeatValve_Pct = self.heat_demand_pct
            self.heat_demand = self.heat_demand_pct > 0.0
        elif s == State.FROST:
            self.fan_demand_pct = 0.0
            io.DO_FreshDamper_Open = False
            io.AO_DamperExhaust_Pct = 0.0
...
class PlantSimulator:
    t_outdoor: float = 0.0           # rooftop outdoor air
    t_supply: float = 8.0
    t_room: float = 16.0
    t_water_return: float = 35.0
    damper_lag_s: float = 0.0        # time accumulator for damper opening
    damper_fail: bool = False        # if True, damper never proves open
    fault_inject: bool = False

    def step(self, io: IOImage, dt_s: float) -> None:
        # 2-point fresh-air damper: proves open ~3 s after commanded (unless failed)
        ...
        target_supply = self.t_outdoor + heat * 30.0
        if flow > 0.01:
    ...
    while cycles is None or n < cycles:
        ...
        if n % 25 == 0:
            print(f"t={n * P.DT_S:6.1f}s {ctrl.state.name:8s} "
                  ...
                  f"FreshDmp={int(io.DO_FreshDamper_Open)}/{int(io.DI_FreshDamper_Open)} "
                  f"Heat={io.AO_HeatValve_Pct:5.1f}% Alarm={int(io.DO_CommonAlarm)}")
...
if __name__ == "__main__":
    print("AHU_08 (RoofTop_Heating) soft-PLC starting (200 ms cycle, Ctrl+C to stop)\n")
    try:
