...
"""
================================================================================
 AHU_07 - Air Handling Unit Control (Python soft-PLC)
          group "Circulation_Clean_Hepa"
================================================================================
 Functional alternative to the licensed CoDeSys 3.5 ST program (AHU_07.st),
 mirroring the same logic on a 200 ms cycle.

 Source : 10ahus_selection_AHU7.json  (project "10AHUs")

 Unit type: recirculation CLEANROOM unit with HEPA filtration. Room air is drawn
 back, filtered (pre-filter + HEPA), conditioned and returned to the room.
   - one supply VFD fan, run at CONSTANT volume (air changes / HEPA face velocity)
   - one relief/recirculation damper
   - water heating coil (3-way) with frost protection + pump
   - cooling coil (2-way)
   - pre-filter + HEPA dP monitoring
   - sensors: supply T/RH, return T/RH, room T/RH, return-water T
 No outdoor sensor, no exhaust fan, no heat recovery.

 HEPA dP high is raised as an alarm but does NOT stop the unit (keeps cleanroom
 pressurisation); the pre-filter dP is a warning only.
================================================================================
"""
...
class IOImage:
    ...
    DI_FilterInlet_Dirty: bool = False     # pre-filter (warning)
    DI_FilterHepa_High: bool = False       # HEPA dP high (alarm)
    DI_Reset: bool = False
    DI_RemoteEnable: bool = True

    # ---- Analog inputs ----
    AI_T_Supply_C: float = 18.0
    AI_RH_Supply_pct: float = 45.0
    AI_T_Return_C: float = 22.0
    AI_RH_Return_pct: float = 45.0
...
class AHUController:
    ...
    def scan(self, io: IOImage, dt_s: float = P.DT_S) -> None:
        ...
        self.filter_warn = io.DI_FilterInlet_Dirty
        self.hepa_alarm = io.DI_FilterHepa_High
        ...
        # 7) output mapping per state (constant-volume fan)
        s = self.state
        if s in (State.OFF, State.SHUTDOWN):
            self.fan_demand_pct = 0.0
            io.AO_Damper_Pct = 0.0
            io.AO_HeatValve_Pct = io.AO_CoolValve_Pct = 0.0
        elif s == State.STARTUP:
            io.AO_Damper_Pct = 100.0
...
class PlantSimulator:
    t_room: float = 24.0             # recirculated room air
    t_supply: float = 20.0
    t_water_return: float = 35.0
    internal_gain_kps: float = 0.012  # cleanroom equipment heat gain (K/s)
    ...
    def step(self, io: IOImage, dt_s: float) -> None:
        ...
        # recirculation: coils act on returning room air
        target_supply = self.t_room + heat * 22.0 - cool * 20.0
        if flow > 0.01:
        ...
        # constant internal heat gain pushes the room up
        self.t_room += self.internal_gain_kps * dt_s

        if io.DO_HeatPump_Run:
            self.t_water_return += (20.0 + heat * 45.0 - self.t_water_return) * 0.15 * dt_s
        else:
            self.t_water_return += (15.0 - self.t_water_return) * 0.02 * dt_s

        io.AI_T_Supply_C = self.t_supply
        io.AI_T_Return_C = self.t_room
    ...
    while cycles is None or n < cycles:
        ...
        if n % 25 == 0:
            print(f"t={n * P.DT_S:6.1f}s {ctrl.state.name:8s} "
                  ...
                  f"{'HEPA! ' if ctrl.hepa_alarm else ''}Alarm={int(io.DO_CommonAlarm)}")
...
if __name__ == "__main__":
    print("AHU_07 (Circulation_Clean_Hepa) soft-PLC starting "
